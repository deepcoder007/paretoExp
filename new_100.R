...
#  rapply( z_2_t, f=function(x) ifelse(is.nan(x),0,x), how="replace" )
#  rapply( z_1_t, f=function(x) ifelse(is.nan(x),0,x), how="replace" )
...
for(iii in 1:100 ) {
  ...
  while( abs(err) > 0.00001 )
  {
    ...
    while(1) {
      ...
      for ( j in 1:N )  # for all the X_i
      {
        z[j] <- max( ((X[j,1]-theta[zz,4])/theta_curr[6]),  
                     ((X[j,2]-theta[zz,5])/theta_curr[7]) )
      }
