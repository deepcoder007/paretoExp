...
for(iii in 1:100)  {
  # The theta_0 vector with theta_0 = (alpha_0,alpha_1,alpha_2,mu_1,mu_2,sigma_1,sigma_2)
  theta <- data.frame(runif(1,3,9),runif(1,3,9),runif(1,3,9),min(X$X1),min(X$X2),runif(1,3,9),runif(1,3,9))
  theta <- rbind(theta,c(runif(1,3,9),runif(1,3,9),runif(1,3,9),min(X$X1),min(X$X2),runif(1,3,9),runif(1,3,9)))
  colnames(theta)<-c("alpha_0","alpha_1","alpha_2","mu_1","mu_2","sigma_1","sigma_2")
  i<-2
  err<-100000000
