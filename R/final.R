...
Q_theta <- function( theta_t, X) {
  ...
  for(i in 1:n) 
  {
    p <- 1
    if( z_1_t[i] > z_2_t[i] ) {
      p <- ((theta_t$alpha_2*(theta_t$alpha_0+theta_t$alpha_1)/(theta_t$sigma_1*theta_t$sigma_2))*((1+(X[i,1]-theta_t$mu_1)/theta_t$sigma_1)**(-1-theta_t$alpha_0-theta_t$alpha_1))*((1+(X[i,2]-theta_t$mu_2)/theta_t$sigma_2)**(-1-theta_t$alpha_2)))
    } else {
      p <- ((theta_t$alpha_1*(theta_t$alpha_0+theta_t$alpha_2)/(theta_t$sigma_1*theta_t$sigma_2))*((1+(X[i,1]-theta_t$mu_1)/theta_t$sigma_1)**(-1-theta_t$alpha_0-theta_t$alpha_2))*((1+(X[i,2]-theta_t$mu_2)/theta_t$sigma_2)**(-1-theta_t$alpha_1)))
    }
    e <-e+log(p)
  }
  return(e);
...
get_error <- function( theta_t , theta_t_1 , X ) {
  e <- 0 
  n <- dim(X)[1]
  # at time (t)
  z_1_t <-  (X$X1-theta_t$mu_1)/theta_t$sigma_1
  ...
  # at time (t+1)
  z_1_t_1 <- (X$X1-theta_t_1$mu_1)/theta_t_1$sigma_1
  z_2_t_1 <- (X$X1-theta_t_1$mu_2)/theta_t_1$sigma_2
  
  for(i in 1:n) 
  {
    p <- 1
    if( z_1_t_1[i] > z_2_t_1[i] ) {
      p <- (theta_t_1$alpha_2*(theta_t_1$alpha_0+theta_t_1$alpha_1)/(theta_t_1$sigma_1*theta_t_1$sigma_2))*((1+(X[i,1]-theta_t_1$mu_1)/theta_t_1$sigma_1)**(-1-theta_t_1$alpha_0-theta_t_1$alpha_1))*((1+(X[i,2]-theta_t_1$mu_2)/theta_t_1$sigma_2)**(-1-theta_t_1$alpha_2))
    } else {
      p <- (theta_t_1$alpha_1*(theta_t_1$alpha_0+theta_t_1$alpha_2)/(theta_t_1$sigma_1*theta_t_1$sigma_2))*((1+(X[i,1]-theta_t_1$mu_1)/theta_t_1$sigma_1)**(-1-theta_t_1$alpha_0-theta_t_1$alpha_2))*((1+(X[i,2]-theta_t_1$mu_2)/theta_t_1$sigma_2)**(-1-theta_t_1$alpha_1))
    }
    if( z_1_t[i] > z_2_t[i] ) {
      p <- p/((theta_t$alpha_2*(theta_t$alpha_0+theta_t$alpha_1)/(theta_t$sigma_1*theta_t$sigma_2))*((1+(X[i,1]-theta_t$mu_1)/theta_t$sigma_1)**(-1-theta_t$alpha_0-theta_t$alpha_1))*((1+(X[i,2]-theta_t$mu_2)/theta_t$sigma_2)**(-1-theta_t$alpha_2)))
    } else {
      p <- p/((theta_t$alpha_1*(theta_t$alpha_0+theta_t$alpha_2)/(theta_t$sigma_1*theta_t$sigma_2))*((1+(X[i,1]-theta_t$mu_1)/theta_t$sigma_1)**(-1-theta_t$alpha_0-theta_t$alpha_2))*((1+(X[i,2]-theta_t$mu_2)/theta_t$sigma_2)**(-1-theta_t$alpha_1)))
    }
    # print(c(e,p))
    e <- c(e,p)
    print(p)
...
theta<-data.frame(20,20,20,20,20,20,20)
...
while( err > 0.1 )
{
  ...
  sm <- 0
  for(j in 1:N) {
    sm <- sm + 1/(theta[i,6]+X$X1[j]-theta_curr[4])
  }
  if( abs(sm)<0.001 )
  {
  ...
  for(j in 1:N) {
    sm <- sm + 1/(theta[i,7]+X$X2[j]-theta_curr[5])
  }
...
#  print(c("Theta is : ",theta_curr))
  err <-abs(Q_theta(theta[i,],X)-Q_theta(theta[i-1,],X))
